...
class SimpleTFKinematics(Node):
    ...
    def getTransformCallback(self, req, res):
        ...
        reqested_transform = TransformStamped()
        try: 
            reqested_transform = self.tf_buffer_.lookup_transform(req.frame_id, req.child_frame_id, 
                                                              rclpy.time.Time())
        except TransformException as e:
            self.get_logger().error("An error occured while transforming %s and %s" %
                                    (req.frame_id, req.child_frame_id))
            res.success = False
            return res
        
        res.transform = reqested_transform
        res.success = True
        return res
